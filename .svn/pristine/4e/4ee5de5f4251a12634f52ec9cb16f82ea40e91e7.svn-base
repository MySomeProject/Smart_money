...
#import "ZMEmailTableViewCell.h"
#import "ZMPhoneNumVerifyCodeViewController.h"

@interface ZMEmailSettingViewController ()<UITableViewDelegate, UITableViewDataSource, UIAlertViewDelegate, UITextFieldDelegate, MBProgressHUDDelegate>
{
    //昵称输入框
...
@implementation ZMEmailSettingViewController
...
//    [self.tableView addSubview:nextStepButton];
    ...
    reuseCellIndentifier = @"ZMEmailTableViewCell";
    UINib *nib = [UINib nibWithNibName:@"ZMEmailTableViewCell" bundle:nil];
...
//{
//    if(![ZMTools isValidateEmail:textField.text])
//    {
//        [[HUD sharedHUDText] showForTime:1.5 WithText:@"邮箱格式不正确!"];
//        return;
//    }
//    
...
{
    ZMEmailTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:reuseCellIndentifier];
    
    if (!cell) {
        cell=[[ZMEmailTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseCellIndentifier];
    }
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    cell.nextSetupBlock = ^(){
        if (!self.phoneNumVCController)
        {
            self.phoneNumVCController = [[ZMEmailVerifyCodeViewController alloc] init];
        }
