...
@implementation ZMEmailTableViewCell
...
- (IBAction)nextSetupButtonClicked:(id)sender
{
    ...
    [progressHUD setLabelText:@"正在验证绑定邮箱..."];
    [[ZMServerAPIs shareZMServerAPIs] checkOldEmail:self.mobileTextField.text andIdentifyCode:self.identifyCodeTextField.text Success:^(id response)
    {
        CLog(@"success");
...
- (void)getIdentifyCodeClicked:(id)sender
{
    ...
    [[ZMServerAPIs shareZMServerAPIs] getIdentifyCodeByOldEmail:self.mobileTextField.text Success:^(id response)
    {
        CLog(@"seccess");
        dispatch_async(dispatch_get_main_queue(), ^(){
            progressHUD.tag = 2001;
    ...
    failure:^(id response)
    {
        ...
        dispatch_async(dispatch_get_main_queue(), ^(){
            progressHUD.tag = 2001;
            [progressHUD hide:YES afterDelay:0];
            if ([errorMessage isEqualToString:@"当前邮箱不是绑定邮箱"])
            {
                UIAlertView *alter = [[UIAlertView alloc] initWithTitle:@"温馨提示" message:errorMessage delegate:nil cancelButtonTitle:nil otherButtonTitles:@"好的", nil];
                [alter show];
            }
