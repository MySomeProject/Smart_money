//
//  BankCardTableViewControllerForRecharge.m
//  ZiMaCaiHang
//
...
@interface BankCardTableViewControllerForRecharge ()
{
...
@implementation BankCardTableViewControllerForRecharge
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch (indexPath.section) {
        case 0:
        {
            ...
            //处于充值模式 add by 张小辉 2015-06-8
            cell.isForRechargeMoney = YES;
            //处于充值模式 end
            ...
            if(indexPath.row == (bankCardArray.count - 1))
            {
                ...
                cell.bottomLongBottomSpace.constant = 2;
            }
            
            NSDictionary * userbankInfo = [bankCardArray objectAtIndex:indexPath.row];
            
            CLog(@"XXXXXX userbankInfo %@", userbankInfo);
...
//            [cell.selectedImage setBackgroundColor:[UIColor redColor]];
...
//                    }
                    ...
                    cell.nextStepButton.enabled = YES;
                    [cell.nextStepButton setBackgroundColor:Color_of_Purple];
                }
                else
                {
                ...
                __weak BankCardTableViewControllerForRecharge *weakself = self;
...
//            进入银行卡详情页面
           self.selectedCell = (BankCardInfoCell *)[self.tableView cellForRowAtIndexPath:indexPath];
            ...
            {
                //选中显示勾勾
                self.selectedCell.selectedImage.hidden = NO;
                
                //选中银行
                selectedUserBankInfo = [bankCardArray objectAtIndex:indexPath.row];
                CLog(@"选中银行: selectedUserBankInfo %@", selectedUserBankInfo);
            }
        }
            break;
