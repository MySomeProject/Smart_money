...
#import "ZMEmailVerifyTableViewCell.h"
#import "ZMSafeSettingsViewController.h"
//验证码计时
#define COUNT_DOWN_NUMBER      60
static int countdownNumber = COUNT_DOWN_NUMBER;
@implementation ZMEmailVerifyTableViewCell

- (void)awakeFromNib {
    // Initialization code
    self.mobileTextField.delegate = self;
    self.mobileTextField.keyboardType = UIKeyboardTypeEmailAddress;
    self.mobileTextField.tag = 1000;
    self.identifyCodeTextField.delegate = self;
    self.identifyCodeTextField.tag = 1001;
    self.identifyCodeTextField.keyboardType = UIKeyboardTypeEmailAddress;
    self.getIdentifyButton.enabled = NO;
    self.nextSetupButton.enabled = NO;
    [self.mobileTextField becomeFirstResponder];
    [self createIdentifyButton]; UITapGestureRecognizer *gesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapGestureToHideInputBoard)];
...
{
    if (![ZMTools isValidateEmail:[self.mobileTextField text]])
    {
        UIAlertView *alter = [[UIAlertView alloc] initWithTitle:@"温馨提示" message:@"邮箱格式不正确" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"好的", nil];
        [alter show];
        return;
    }
    ...
    [progressHUD setLabelText:@"正在绑定邮箱..."];
    [[ZMServerAPIs shareZMServerAPIs] rebindEmailWithNewEmail:self.mobileTextField.text andIdentifyCode:self.identifyCodeTextField.text Success:^(id response)
     {
         CLog(@"success");
         dispatch_async(dispatch_get_main_queue(), ^(){
             ...
             alter.delegate = self;
             [alter show];
             [ZMAdminUserStatusModel shareAdminUserStatusModel].emailValidated = YES;
         });
     }
    failure:^(id response)
     {
...
- (void)getIdentifyCodeClicked:(id)sender
{
    ...
    [[ZMServerAPIs shareZMServerAPIs] getIdentifyCodeByNewEmail:self.mobileTextField.text Success:^(id response)
     {
         CLog(@"seccess");
         dispatch_async(dispatch_get_main_queue(), ^(){
             progressHUD.tag = 2001;
